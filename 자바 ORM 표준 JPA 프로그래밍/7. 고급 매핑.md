...
영속성 컨텍스트는 엔티티의 식별자를 키로 사용해서 엔티티를 관리합니다. 그리고 식별자를 비교할 때 equals()와 hashCode()를 사용합니다. 따라서 식별자 객체의 동등성이 지켜지지 않으면 예상과 다른 엔티티가 조회되거나 엔티티를 찾을 수 없는 등 영속성 컨텍스트가 엔티티를 관리하는 데 심각한 문제가 발생합니다. 따라서 복합 키는 equals()와 hashCode()를 필수로 구현해야 합니다.
